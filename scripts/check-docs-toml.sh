...
#
# WHY THIS EXISTS: the corpus gates the other things a reader copies off a page.
# `scripts/check-docs-links.sh` gates its *links*, so nobody is sent to a page
# that does not exist; `scripts/check-docs-cli.sh` gates its *commands*, so
# nobody is handed an `autumn …` line clap will reject; and
# `scripts/check-docs-config.sh` gates the `AUTUMN_*` environment variables a
# reader is told to SET. This gates the one surface left: the config they
# WRITE — a key in `autumn.toml` itself.
#
# THE TWO CONFIG GATES DO NOT OVERLAP. The env layer is written field by field,
# so a schema leaf can exist with no environment spelling at all (90 of 397 do),
# and a `[section] key` inside a fence is never an `AUTUMN_*` name. One gate
# resolves names the runtime reads; this one resolves keys the schema has.
#
# It is the same silent-failure class as the env gate, one layer over — with a
# wrinkle worth stating exactly, because it is the reason this gate is needed at
# all. `server.strict_config` and `autumn check --config` DO reject an unknown
# key in `autumn.toml`. But `strict_config` is `false` by default (`config.rs`,
# `ServerConfig`), so on a default app serde drops the key without a warning:
# the app boots, the setting the reader believed they changed keeps its default,
# and nothing anywhere says so. A bad link 404s and a bad command exits 2 — both
# are dead ends the reader can see and route around. A bad config key in a file
# nobody validates is a dead end they cannot see.
#
# The reader-facing corpus carries 172 `autumn.toml` fences judged against 484
# schema leaves, and a renamed or never-shipped key leaves behind a line that
# looks exactly like a working one. The baseline found four live defects:
#
#   `docs/guide/wizards.md` — under "consider increasing the session TTL in
#   `autumn.toml`", a fence writing `[session] ttl_seconds = 3600`. There is no
#   `ttl_seconds`: the key is `session.max_age_secs` and its default is `86400`,
#   so the documented "increase" was also a 24x DECREASE if the reader
#   hand-corrected the name. Searching the guide for the asker's own words,
#   "session ttl", returned that one page and no other — simultaneously the only
#   page a reader could land on for the question and the wrong answer to it.
#
#   `docs/guide/dev-error-overlay.md` — "set the profile to production" over a
#   fence marked `# autumn.toml` writing `[app] profile = "production"`. There is
#   no `[app]` section, and `AutumnConfig::profile` is `#[serde(skip)]` —
#   "Resolved at load time, not deserialized from TOML" — so the profile cannot
#   be set from the file under ANY section name. It traces to ADR 0006, which
#   proposed exactly that "one-line opt-out in `autumn.toml`"; the implementation
#   resolved the profile from `AUTUMN_ENV`/`--profile`/build mode instead and the
#   guide shipped the proposal.
#
#   `docs/guide/cloud-native.md` — `read_your_writes` set twice in one
#   `[database]` table, under prose telling the reader to add it. A TOML
#   duplicate-key error: the app does not start.
#
#   `examples/wiki/content/configuration.md` — `[logging] level` for the root
#   that is `[log]`. This file is `include_str!`'d into the wiki example and
#   served at `/docs/configuration`, so it is reader-facing documentation.
#
...
#      widening.
#
# WHICH FENCES ARE READ. Of the corpus's 253 ```toml fences, 172 are read. A
...
#
# USAGE:
#   scripts/check-docs-toml.sh              # gate the corpus
#   scripts/check-docs-toml.sh --list       # every fence read, and its verdict
#   scripts/check-docs-toml.sh --self-test  # synthetic-corpus tests
...
Inspect what the gate read:  scripts/check-docs-toml.sh --list
EOF
      exit 1
    fi
    ;;
