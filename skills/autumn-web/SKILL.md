...
| `active_search(id, label, &ActiveSearchConfig)` / `active_search_empty_state(msg)` | Search box whose results appear as the user types — htmx-driven, server-rendered Maud fragments, no client-side search library. The handler returns the results fragment; `active_search_empty_state` is the no-matches body (`docs/guide/active-search-and-autocomplete.md`) |
| `autocomplete_input(id, label, &AutocompleteConfig)` / `autocomplete_option(value, label)` / `autocomplete_empty_state(msg)` | Typeahead/autocomplete picker for choosing a related record and storing its ID — the companion to `active_search` when the user must pick one row rather than browse matches (`docs/guide/active-search-and-autocomplete.md`) |
