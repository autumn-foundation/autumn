...
- [Active Search & Autocomplete](docs/guide/active-search-and-autocomplete.md) — a search box whose results appear as you type, and typeahead/autocomplete pickers that store the chosen record's ID, via `active_search` and `autocomplete_input`; built on htmx and server-rendered Maud fragments, so you write no JavaScript of your own — active search needs htmx, and autocomplete selection also needs the shipped `/static/js/autumn-widgets.js` runtime. Both emit a `<noscript>` form fallback
