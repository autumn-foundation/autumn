...
| ISR regenerates a route whose handler now declares a different type; the manifest still says the old one. | The manifest is immutable at runtime, so the new bytes would be served under the old header. `regenerate_page` **refuses** the write, leaving the previous stale-but-correctly-typed file in place; a rebuild re-records the type. (First drafted as a `warn!`-only check; Codex review correctly pointed out that logging does not prevent the desync, and that a handler which drops the header entirely bypasses a mismatch check.) |
