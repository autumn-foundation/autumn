...
— and the table below is generated from it. `scripts/check-plugin-surface.sh`
fails CI when the two disagree, so this page cannot quietly go stale.
...
| `AppBuilder::plugin_migrations` | stable | Contribute embedded database migrations tagged with the plugin's own name (requires the `db` feature). |
...
| `AppBuilder::with_edge_kv` | experimental | Edge-capsule KV binding. The whole edge lane (issue #1790) may change in any release; the capsule wire protocol carries its own version field. |
...
every minor bump is breaking, so a first-party plugin declares its own minor
series; from `1.0` on, the major.
...
**Declaring nothing is still valid.** `Plugin::contract` defaults to `None`,
and a plugin that returns `None` behaves exactly as it did before the contract
existed. It just gets no diagnostic when it is mounted into a framework it was
not written for.
...
```text
plugin `autumn-plugin-example 0.6.2` supports autumn-web 0.6, but this
application builds against autumn-web 0.7.0.
  → upgrade the plugin to a release built for autumn-web 0.7.0
    (`cargo update -p autumn-plugin-example`), or
...
mod conformance_tests {
    ...
    fn plugin_passes_conformance() {
        ...
        let config = ConformanceConfig::new("autumn-myplugin-plugin")
            ...
            // Pass the plugin's own contract so the experimental-surface check
            // runs instead of being skipped.
            .contract(MyPlugin::new().contract().expect("a contract"));
