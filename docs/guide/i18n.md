...
- A stored value that is *not* a JSON object of strings is read as the default
  ...
  `#[translatable]` with **no data migration** — the flip side being that a
  column already holding a JSON string-map of something else would be read as
  translations. Declare it only on columns that hold human-readable text.
