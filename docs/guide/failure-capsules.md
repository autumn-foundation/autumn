...
| Credentials *inside* a masked header | the token after an auth scheme (`Bearer …`), what a `Basic` credential decodes to (the `user:password` pair and the password alone), each value of an auth-param list (`Signature=…`), and each cookie value join the echo set on their own, because that is the form a handler extracts and may echo. A cookie value joins it under **both** spellings — as it arrived and percent-decoded (`abc%2Fdef` and `abc/def`) — because decoding a cookie value before use is a common convention that Autumn's own time-zone cookie follows; `+` stays literal, since a cookie is not form-encoded. The decoded spelling is recorded only when it is at least four bytes and not all whitespace: `%2F` and `%3D` decode to needles that would rewrite `failed at /` and `x = y` in the outcome and blank unrelated binds. Auth-param values are masked only where they stand as whole tokens, since the list mixes secrets with metadata (`qop=auth`) that would otherwise shred prose; cookie values are whole-token-only for the same reason. Usernames and cookie *names* are not recorded at all — they are ordinary words. The whole header is parsed as bytes, so one `obs-text` byte inside a quoted `Digest` parameter no longer discards the ASCII parameters beside it |
