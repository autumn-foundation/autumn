...
* **Exponential Backoff**: the job engine computes `initial_backoff_ms * 2^(attempt-1)` (`local_retry_delay_ms` in `autumn/src/job.rs`), so the default 1000 ms base gives 1 s, 2 s, 4 s, 8 s.
* **Jitter**: applied to that delay as **equal jitter**, which *reduces* it rather than adding to it: the scheduled delay is a random value in `[delay/2, delay]`, so the first retry lands 500-1000 ms after the failure. Half is guaranteed so a retry can never fire near-instantly, and the delay never exceeds the un-jittered value — which is why it cannot push a retry past an existing timeout budget.
