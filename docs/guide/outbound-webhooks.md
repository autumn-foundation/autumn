...
- **Autumn transmits no event or delivery identifier.** A delivery carries `Content-Type` and `Autumn-Signature` and nothing else, and that signature's `t=` timestamp is regenerated on every attempt. If a receiver must deduplicate, the application has to mint a stable ID, put it in the payload, and reuse it verbatim on every retry.
