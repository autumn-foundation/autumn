...
3. **`WebhookOutboundManager`**: The central coordinator available via `AppState` extensions, providing the `.dispatch()` method, which writes a delivery-log row per subscription and then enqueues the delivery job. The two steps are ordered, not atomic: no transaction spans the pluggable store and the job queue, so a crash between them can leave a logged event that was never enqueued. This is not an outbox guarantee, and it cannot be turned into one by reconciling after the fact — a *successful* enqueue writes no marker on the log row, so a row that was enqueued is indistinguishable from one whose process died first. A sweeper over those rows must either re-enqueue (duplicating deliveries) or skip (dropping them); it cannot tell which is correct. **Implementing the trait does not close this window.** `OutboundWebhookHandler` exposes storage methods only; `dispatch()` calls `log_delivery` and performs the enqueue *afterwards*, outside the trait, so no implementation can bring the enqueue into its own transaction. Closing the window inside the manager would require a framework change. If you need a stronger guarantee today, keep the durable record outside this subsystem: write your own outbox row in the same transaction as the business data that triggered the event, and have a worker call `dispatch()` for unprocessed rows and mark them processed only once it returns `Ok`. That never loses an event, but it can deliver one twice. Deduplicating on the receiver takes one more step, because **Autumn transmits no event or delivery identifier**: a delivery carries `Content-Type` and `Autumn-Signature` and nothing else, and that signature's `t=` timestamp is regenerated on every attempt, so there is nothing framework-provided to key on. Put a stable ID inside the payload you hand to `dispatch()` — minted in the outbox transaction and reused verbatim on every retry — and have receivers deduplicate on that. Receivers should be idempotent regardless — but note what that does and does not buy you: retries can deliver an event more than once *only after the job is enqueued*. Before that point the loss window above still applies, so calling `dispatch()` directly is not an at-least-once guarantee either. Idempotent receivers protect you from duplicates; only a durable record you control protects you from loss. One case is handled for you, on the default path only: when `dispatch()` falls back to enqueuing the `autumn_webhook_delivery` job and that enqueue fails, the log row is marked `is_dlq` and can be replayed from the DLQ endpoints below. If a `WebhookDelegateExt` is installed, `dispatch()` hands the delivery to that delegate *instead* of enqueuing, and a delegate error is returned to the caller without marking the row — so a failed delegated delivery never appears in the DLQ, and an operator looking there will not find it.
