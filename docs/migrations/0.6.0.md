...
diff and `autumn upgrade --apply` to write it, then read each flagged site: the
receiver check is textual, and can be fooled by a hand-written type or a
`#[repository]` from another crate (issue #2234). Call sites inside a macro
