...
/// Parse a handler function from macro input that may ALSO carry a guard's
/// `FromRequestParts` gate `struct`/`impl` pair ahead of it.
///
/// `#[throttle]`, `#[secured]`, and `#[step_up]` each end their own expansion
/// with `quote! { #gate_struct #gate_impl #handler_fn }` — a 3-item stream,
/// not a single function — because each macro's session/step-up/rate-limit
/// check now runs inside a hidden `FromRequestParts` gate type inserted as
/// the handler's first parameter, rather than as a body statement (this lets
/// Axum reject before the request body is ever parsed). So whenever one of
/// these macros is written ABOVE another guard or a route attribute — making
/// it expand first — the outer macro receives that multi-item stream as its
/// own `item`, and stacking two guards nests this a level deeper still (the
/// second gate's `item` is itself a `[gate, gate, fn]` triple). Every macro
/// that might sit outside another must find the handler fn inside whatever
/// it's handed, AND re-emit every other item verbatim in its own output —
/// dropping one would leave some gate parameter referencing a type that no
/// longer exists anywhere in the expansion.
///
/// Returns `(leading_items, handler_fn)`: `leading_items` is empty (and
/// re-emitting it is a no-op) for the common case of a bare handler, or one
/// whose guard attributes are still unexpanded markers below this one.
/// Callers validate `handler_fn.sig.asyncness` themselves so each keeps its
/// own error wording (`"#[secured] can only be applied to async functions"`
/// vs. the route macros' own message).
pub fn parse_multi_item_handler(item: TokenStream) -> Result<(TokenStream, ItemFn), TokenStream> {
    // Fast path: a bare handler, or one whose guard attributes are still
    // markers below this macro (unexpanded), parses as a single item.
    if let Ok(input_fn) = syn::parse2::<ItemFn>(item.clone()) {
        return Ok((TokenStream::new(), input_fn));
    }

    // Slow path: a guard macro already expanded above this one and handed us
    // its gate struct/impl ahead of the handler fn (or ahead of an even
    // earlier guard's own gate pair, when guards stack).
    ...
    for item in items {
        match item {
            syn::Item::Fn(f) if handler_fn.is_none() => handler_fn = Some(f),
            other => quote::ToTokens::to_tokens(&other, &mut leading),
        }
    }

    let Some(input_fn) = handler_fn else {
        return Err(syn::Error::new_spanned(
            leading,
