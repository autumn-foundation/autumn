...
impl Plugin for BillingPlugin {
    ...
    fn build(self, app: AppBuilder) -> AppBuilder {
        ...
        let app = app
            .nest(&config.route_prefix, routes::router(&config))
...
/// Fail boot when the app did not declare the webhook receiver.
fn verify_webhook_endpoint(state: &AppState, config: &BillingConfig) -> Result<(), AutumnError> {
    ...
    Err(AutumnError::internal_server_error_msg(format!(
        ...
        config::STRIPE_WEBHOOK_SECRET_ENV
    )))
}
