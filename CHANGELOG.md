...
- **🗃️ Ledger: batch `examples/cms`'s search/listing permalink resolution
  (statements 16-80→0, buffers -32%..-69%):** `Repos::permalink`
  ...
  every result's own row plus every ancestor across the whole batch, and
  the existing pure `site::permalink_from` builds each URL from that map
  with no further database access (and no connection checkout at all when
  no result is `page`-typed — the common case for `blog_index`/
  `term_archive`/`author_archive`/`date_archive`, which only ever list
  `post_type = "post"` results). Profiled through the real `/search` route
  ...
  ancestor/self-row work down 31.6%/60.0%/68.9%. No behavior change: a new
  equivalence test
