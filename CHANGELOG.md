...
- **repository:** **Breaking:** `#[repository(api = "...", owner = column)]`
  with no `policy = Type` or `scope = Type` is now a compile error (🛡
  Warden). `owner = <column>` only emits opt-in `list_scoped(owner_id, ..)` /
  ...
  never call them, and only branch on `policy`/`scope`. Declared on its own
  next to `api = "..."`, `owner` therefore compiled to a fully public REST
  API that read, at the declaration site, like a per-owner-scoped one: `GET
  <api>` returned every user's rows (`repository_macro`'s own list-body
  precedence only considers `scope`/`policy`/neither — never `owner_column`),
  and `GET`/`PUT`/`DELETE <api>/{id}` let any authenticated caller
  read/overwrite/delete any other user's row by id, since `find_by_id`/
  `update`/`delete_by_id` are not owner-filtered either. **Migration:** add
  `policy = Type` (whose `can_show`/`can_update`/`can_delete` can compare
  `ctx.user_id_i64()` against the owner column) or `scope = Type` alongside
