...
- **🗃️ Ledger: batch the `dependent(on_delete = destroy)` leaf cascade
  (statements 10002→2, buffers -88.8%):** the generated `Destroy` cascade
  ...
  even when the child has no `hooks`, isn't `soft_delete`/`versioned`, and
  declares no `dependent(...)`/`#[has_many(dependent = ...)]` of its own,
  so nothing in that configuration ever read the reload. That exact
  configuration now routes through the same `dependent_delete_all` runtime
  helper `on_delete = delete_all` already uses (which already batches its
  optional counter-cache decrement), guarded by a cheap runtime check for
  model-attribute grandchildren invisible to the macro's own attributes.
  Every other configuration (hooks, soft-delete, versioned, broadcasting,
  or any grandchildren) is unaffected and keeps the exact prior per-row
  loop. Profiled against a 500-post/~23k-comment fixture cascading a
  5,000-comment delete: `pg_stat_statements` calls 10,002 → 2, buffers
  45,583 → 5,126. Full existing `dependent`/`has_many` suite (29 tests,
  including diamond-cascade and hook/soft-delete cases) passes unchanged.
  See `docs/reports/2026-09-08-ledger-dependent-destroy-leaf-batch/`.
