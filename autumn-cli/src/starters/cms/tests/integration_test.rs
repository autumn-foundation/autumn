...
async fn a_nested_custom_type_item_is_still_reachable() {
    ...
    {{crate_name}}::content_types::register_post_type({{crate_name}}::content_types::PostType {
        hierarchical: true,
        archive_base: "runbooks",
        ..{{crate_name}}::content_types::PostType::new("runbook", "Runbook", "Runbooks")
    })
...
async fn a_hierarchy_edit_settles_every_descendant_path() {
    ...
    let paths = {
        let mut conn = TestDb::shared().await.pool().get().await.expect("conn");
        {{crate_name}}::content::page_paths_under(&mut conn, old)
            .await
            .expect("the paths")
    };
...
async fn an_export_carries_comments_and_an_import_restores_them() {
    ...
    let (approved, pending, spam, nested): (i64, i64, i64, i64) = {
        ...
        let count = async |conn: &mut _, status: &'static str| -> i64 {
            RunQueryDsl::get_result(
                {{crate_name}}::schema::comments::table
                    .filter({{crate_name}}::schema::comments::status.eq(status))
                    .count(),
                conn,
            )
            .await
        ...
        let nested: i64 = RunQueryDsl::get_result(
            {{crate_name}}::schema::comments::table
                .filter({{crate_name}}::schema::comments::parent_id.is_not_null())
                .count(),
            &mut conn,
        )
        .await
    ...
    let count: i64 = {
        ...
        RunQueryDsl::first(
            {{crate_name}}::schema::posts::table
                .filter({{crate_name}}::schema::posts::slug.eq("discussed"))
    ...
    let total: i64 = {
        ...
        use diesel_async::RunQueryDsl;
        let mut conn = TestDb::shared().await.pool().get().await.expect("conn");
        RunQueryDsl::get_result({{crate_name}}::schema::comments::table.count(), &mut conn)
            .await
            .expect("the count")
    };
...
async fn a_menu_slug_race_takes_the_next_suffix() {
    ...
    let creating = tokio::spawn(async move {
        let mut conn = TestDb::shared().await.pool().get().await.expect("conn");
        {{crate_name}}::content::replace_menu_at_location(&mut conn, "Main", "").await
    });
...
///
/// See [`{{crate_name}}::routes::admin::posts`]'s `PostForm::validate_fields`: the same
...
async fn scheduling_a_post_with_no_publish_date_redisplays_the_editor() {
    ...
    use diesel_async::RunQueryDsl;
    let mut conn = TestDb::shared().await.pool().get().await.expect("conn");
    let count: i64 = {{crate_name}}::schema::posts::table
        .count()
        .get_result(&mut conn)
        .await
        .expect("count");
...
async fn rescheduling_an_edit_with_no_publish_date_redisplays_the_editor() {
    ...
    use diesel_async::RunQueryDsl;
    let mut conn = TestDb::shared().await.pool().get().await.expect("conn");
    let stored_title: String = {{crate_name}}::schema::posts::table
        .find(id.parse::<i64>().expect("id"))
        .select({{crate_name}}::schema::posts::title)
        .first(&mut conn)
        .await
        .expect("the post");
    assert_eq!(
...
fn a_taxonomy_slug_must_be_one_url_segment() {
    let refused = {{crate_name}}::content_types::register_taxonomy({{crate_name}}::content_types::Taxonomy {
        slug: "product/type",
...
async fn deleting_an_author_takes_the_hierarchy_lock() {
    ...
    // test discriminating rather than a description.
    {
        ...
        diesel::sql_query(format!(
            ...
            {{crate_name}}::content::PAGE_HIERARCHY_LOCK_KEY
        ))
        .execute(&mut holder)
        .await
...
async fn a_malformed_email_cannot_be_saved_on_an_existing_account() {
    ...
    let stored: String = {
        ...
        {{crate_name}}::schema::users::table
            .find(2_i64)
            .select({{crate_name}}::schema::users::email)
            .first(&mut conn)
            .await
            .expect("the account")
    };
...
async fn a_comment_is_refused_when_the_password_moved_under_the_lock() {
    ...
    {{crate_name}}::content::create_comment(
        &mut TestDb::shared().await.pool().get().await.expect("conn"),
        {{crate_name}}::models::NewComment {
            post_id,
            parent_id: None,
...
async fn a_menu_assigned_to_the_footer_is_rendered() {
    ...
    let menu: i64 = {
        ...
        let mut conn = TestDb::shared().await.pool().get().await.expect("conn");
        {{crate_name}}::schema::menus::table
            .filter({{crate_name}}::schema::menus::location.eq("footer"))
            .select({{crate_name}}::schema::menus::id)
            .first(&mut conn)
            .await
            .expect("the menu")
...
async fn seeding_skips_a_slug_the_other_bare_type_already_holds() {
    ...
    {
        let mut conn = TestDb::shared().await.pool().get().await.expect("conn");
        {{crate_name}}::seed::seed_posts(&mut conn, author, chrono::Utc::now().naive_utc())
            .await
    ...
    let taken: i64 = {
        ...
        let mut conn = TestDb::shared().await.pool().get().await.expect("conn");
        {{crate_name}}::schema::posts::table
            .filter({{crate_name}}::schema::posts::slug.eq("hello-world"))
            .count()
            .get_result(&mut conn)
            .await
            .expect("the count")
...
async fn an_import_never_restores_into_the_trash() {
    ...
    let status: String = {
        ...
        let mut conn = TestDb::shared().await.pool().get().await.expect("conn");
        {{crate_name}}::schema::posts::table
            .filter({{crate_name}}::schema::posts::slug.eq("deleted-elsewhere"))
            .select({{crate_name}}::schema::posts::status)
            .first(&mut conn)
            .await
...
async fn the_hierarchy_lock_comes_before_any_post_row_lock() {
    ...
    let trash = tokio::spawn(async move {
        let mut conn = TestDb::shared().await.pool().get().await.expect("conn");
        {{crate_name}}::content::transition_status(&mut conn, post_id, "trash", None, None).await
    });
