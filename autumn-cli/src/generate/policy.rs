...
pub fn plan_policy(project_root: &Path, name: &str) -> Result<Plan, GenerateError> {
    ...
    push_policy_files(&mut plan, project_root, &pascal_name, &snake_name, &plural, owner.as_deref());
...
pub(super) fn detect_owner_column(columns: &[String], user_ref_columns: &[String]) -> Option<String> {
...
fn render_policy_file(pascal_name: &str, snake_name: &str, plural: &str, owner: Option<&str>) -> String {
...
"#
    ...
    fn user_id_owner_generates_ownership_check() {
        ...
        assert!(!src.contains("TODO"), "owner case must not carry TODO: {src}");
    }
    ...
    fn builder_registration_inserted_exactly_once_when_run_twice() {
        ...
        let main = fs::read_to_string(tmp.path().join("src/main.rs")).unwrap();
        assert_eq!(
            main.matches(".policy::<crate::models::post::Post, _>").count(),
            1,
            "policy registration must appear exactly once: {main}"
        );
        assert_eq!(
            main.matches(".scope::<crate::models::post::Post, _>").count(),
            1,
