...
fn render_api_smoke_test(
    ...
    let seed_sql = render_api_seed_sql(plural, fields, defaults);
    let escaped_seed = escape_sql_for_rust_literal(&seed_sql);
    format!(
        "//! Smoke test generated by `autumn generate scaffold --api`.\n\
         ...
         db.execute_sql(\"{escaped_seed}\").await;\n\
         \n\
...
/// defaulted, e.g. a fieldless scaffold), it degenerates to a targetless
/// `INSERT ... SELECT FROM generate_series`, which inserts N all-default rows
/// — the case the old `INSERT ... DEFAULT VALUES` covered.
fn render_api_seed_sql(
    ...
    if columns.is_empty() {
        format!("INSERT INTO {plural} SELECT FROM generate_series(1, {n})")
    } else {
        format!(
