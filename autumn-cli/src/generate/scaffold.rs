...
use super::schema_edit::{
    add_mod_declaration, create_table_sql_with_metadata_and_id_for, ensure_autumn_web_feature,
...
fn plan_scaffold_with_options_impl(
    ...
    plan.create(
        ...
        render_smoke_test(
            super::detect_backend(project_root),
            &pascal_name,
            &plural,
            options_with_key.api,
            &smoke_test_fields,
...
/// The statement a smoke test issues to empty its throwaway table before
/// seeding. Postgres uses `TRUNCATE <table> RESTART IDENTITY` (byte-for-byte
/// identical to the historical output); `SQLite` has no `TRUNCATE`, so it uses
/// `DELETE FROM <table>` — enough for a freshly-created throwaway table (issue
/// #1614). Returned without a trailing `;` to match the historical Postgres
/// call sites.
fn reset_table_sql(backend: DatabaseBackend, table: &str) -> String {
    match backend {
        DatabaseBackend::Postgres => format!("TRUNCATE {table} RESTART IDENTITY"),
        DatabaseBackend::Sqlite => format!("DELETE FROM {table}"),
    }
}
...
fn render_index_smoke_test(
    ...
    let reset_table = reset_table_sql(backend, plural);
    format!(
        "//! Smoke test generated by `autumn generate scaffold`.\n\
         ...
         {setup_calls}\
         db.execute_sql(\"{reset_table}\").await;\n\
         \n\
...
fn render_api_smoke_test(
    ...
    // per row; the SQLite required-column branch emits one literal `INSERT` per
    // row) becomes one `db.execute_sql(...)` call apiece -- diesel-async's
    // `execute` uses the extended protocol, which rejects several `;`-separated
    // statements in a single call. The Postgres single-statement required-column
    // branch is unaffected (it renders exactly one call).
    let seed_calls =
        render_execute_sql_calls(&render_api_seed_sql(backend, plural, fields, defaults));
    let reset_table = reset_table_sql(backend, plural);
    // Postgres accepts the `id::text` cast; SQLite has no `::` cast operator, so
    // it uses the portable `CAST(id AS TEXT)` (issue #1614).
    let id_to_text = match backend {
        DatabaseBackend::Postgres => "id::text",
        DatabaseBackend::Sqlite => "CAST(id AS TEXT)",
    };
    format!(
...
fn render_reference_stub_tables_sql(
    backend: DatabaseBackend,
    fields: &[Field],
    own_table: &str,
) -> String {
    // Postgres stub PK stays `BIGSERIAL` (byte-for-byte identical); SQLite has no
    // `BIGSERIAL`, so the stub uses `INTEGER PRIMARY KEY` — a SQLite rowid alias
    // that auto-assigns ids 1, 2, … to the `DEFAULT VALUES` rows below, matching
    // the `references` FK the smoke test points at it (issue #1614).
    let stub_pk = match backend {
    ...
    for f in fields {
        ...
        {
            let _ = writeln!(out, "CREATE TABLE IF NOT EXISTS {target} (id {stub_pk});");
...
fn render_smoke_test(
    ...
    let stub_tables_sql = render_reference_stub_tables_sql(backend, fields, plural);
    let create_table_sql = create_table_sql_with_metadata_and_id_for(
        backend, plural, fields, indexes, defaults, id_type,
    );
    ...
    let base = if api {
        ...
        render_index_smoke_test(backend, pascal_name, plural, id_schema_type, &setup_calls)
    };
...
fn render_api_seed_sql(
    ...
    let required: Vec<&Field> = fields
        .iter()
        .filter(|f| !f.nullable && !defaults.contains_key(&f.name))
        .collect();
    let columns: Vec<&str> = required.iter().map(|f| f.name.as_str()).collect();
    ...
    if columns.is_empty() {
        ...
        // into one `db.execute_sql(...)` call per statement. `INSERT ... DEFAULT
        // VALUES` is valid on both backends, so this branch is backend-agnostic.
        return vec![format!("INSERT INTO {plural} DEFAULT VALUES"); n].join(";\n");
    }
    match backend {
        DatabaseBackend::Postgres => {
            let values: Vec<String> = required.iter().map(|f| sql_seed_value_expr(f)).collect();
            format!(
        ...
        // SQLite has no `generate_series`, so emit N literal INSERTs (one per
        // row, split into one `db.execute_sql(...)` call apiece by
        // `render_execute_sql_calls`), substituting the row index for the
        // Postgres `g` in each per-column sample value.
        DatabaseBackend::Sqlite => (1..=n)
            .map(|g| {
                let values: Vec<String> = required
                    .iter()
                    .map(|f| sqlite_seed_value_literal(f, g))
                    .collect();
                format!(
...
/// A `SQLite`-valid per-row seed literal for a required column in the `--api`
/// smoke test — the `SQLite` counterpart to [`sql_seed_value_expr`]'s
/// Postgres `g`-relative expression, with the row index `g` (1..=
/// [`API_SMOKE_TEST_SEED_ROWS`]) substituted in as a literal since `SQLite` has
/// no `generate_series` to bind `g` to. Only the kinds that survive the
/// upstream `SQLite` field-kind rejection can reach here (`Uuid`, `Decimal`,
/// `DateTime<Utc>`, `Enum`, attachments are already rejected).
///
/// Text/integers/floats/timestamps weave the index in so a `unique` column of
/// one of those types stays distinct across the seeded rows; the constant-value
/// kinds (a `references` FK id, bool, bytea) fall back to a fixed `SQLite`
/// literal — they can't hold [`API_SMOKE_TEST_SEED_ROWS`] distinct values
/// anyway (the same inherent limit [`sql_seed_value_expr`] documents).
fn sqlite_seed_value_literal(f: &Field, g: usize) -> String {
    match f.kind {
        FieldKind::String | FieldKind::Text => format!("'sample-{g}'"),
        FieldKind::I32 | FieldKind::I64 => g.to_string(),
        // No `::double precision` cast on SQLite — `CAST(g AS REAL)` is portable.
        FieldKind::F32 | FieldKind::F64 => format!("CAST({g} AS REAL)"),
        // No `NOW()`/`::interval` on SQLite — a per-row `datetime(...)` offset
        // stays valid and keeps values distinct for a `unique` timestamp column.
        FieldKind::NaiveDateTime => {
            format!("datetime('2000-01-01 00:00:00', '+{g} seconds')")
        }
        // A `references` FK id must be a real stub row (ids 1 and 2 are seeded;
        // see `render_reference_stub_tables_sql`), and bool likewise has no
        // per-row-varying form — both use the constant integer `1` (`true` in
        // SQLite). Bytea falls back to a fixed SQLite blob literal.
        FieldKind::References | FieldKind::Bool => "1".to_owned(),
        FieldKind::Bytea => "X'00'".to_owned(),
        // Unreachable on SQLite (rejected upstream), but keep total: a plain
        // quoted sample keeps generation infallible rather than panicking.
        _ => "'sample'".to_owned(),
    }
}
...
"#;
    ...
    /// `SQLite` foundation (issue #1614 finding F13): the generated `--api`
    /// smoke test must emit `SQLite`-valid SQL for supported field kinds —
    /// `DELETE FROM`
    /// (no `TRUNCATE ... RESTART IDENTITY`), an `INTEGER PRIMARY KEY` reference
    /// stub (no `BIGSERIAL`), `CURRENT_TIMESTAMP` (no `NOW()`), 25 literal
    /// INSERTs (no `generate_series`), `CAST(x AS REAL)` (no `::double
    /// precision`), a per-row `datetime(...)` (no `NOW() + ...::interval`), and
    /// `CAST(id AS TEXT)` (no `id::text`).
    #[test]
    fn plan_scaffold_api_smoke_test_is_sqlite_valid_on_a_sqlite_app() {
        let tmp = project_with_main(default_main());
        fs::write(
        ...
        .unwrap();
        plan.execute(Flags::default()).unwrap();
        let t = fs::read_to_string(tmp.path().join("tests/post.rs")).unwrap();

        assert!(
            t.contains("DELETE FROM posts"),
            "must reset via DELETE: {t}"
        );
        assert!(!t.contains("TRUNCATE"), "no TRUNCATE on SQLite: {t}");
        assert!(!t.contains("generate_series"), "no generate_series: {t}");
        assert!(
            !t.contains("::double precision"),
            "no ::double precision: {t}"
        );
        assert!(
            t.contains("CAST(1 AS REAL)"),
            "float seed uses CAST..REAL: {t}"
        );
        assert!(
            t.contains("datetime('2000-01-01 00:00:00', '+1 seconds')"),
            "datetime seed uses SQLite datetime(): {t}"
        );
        assert!(!t.contains("NOW()"), "no NOW() on SQLite: {t}");
        assert!(
            t.contains("CAST(id AS TEXT)"),
            "id projection uses CAST: {t}"
        );
        assert!(!t.contains("id::text"), "no ::text cast on SQLite: {t}");
        assert!(t.contains("CURRENT_TIMESTAMP"), "created_at default: {t}");
        assert!(!t.contains("BIGSERIAL"), "no BIGSERIAL on SQLite: {t}");
        // 25 literal INSERTs, first row's string literal present.
        assert!(t.contains("'sample-1'"), "literal per-row seed value: {t}");
        assert_eq!(
            t.matches("INSERT INTO posts (title, score, posted_at) VALUES")
                .count(),
            25,
            "expected 25 literal INSERTs (one per row): {t}"
        );
    }
    ...
    fn render_reference_stub_tables_sql_dedupes_identical_targets() {
        ...
        let sql =
            render_reference_stub_tables_sql(DatabaseBackend::Postgres, &fields, "unrelated_table");
        assert_eq!(
    ...
    fn render_reference_stub_tables_sql_seeds_a_row_so_fk_id_1_is_valid() {
        ...
        let sql = render_reference_stub_tables_sql(DatabaseBackend::Postgres, &fields, "posts");
        assert_eq!(
            sql,
    ...
    #[test]
    fn render_reference_stub_tables_sql_sqlite_uses_integer_pk_not_bigserial() {
        // SQLite has no BIGSERIAL; the stub PK must be a plain INTEGER PRIMARY
        // KEY (a rowid alias) so the `DEFAULT VALUES` rows still get ids 1 and 2
        // (issue #1614 finding F13).
        let fields = super::super::dsl::parse_fields(&["author:references".into()]).unwrap();
        let sql = render_reference_stub_tables_sql(DatabaseBackend::Sqlite, &fields, "posts");
        assert_eq!(
            sql,
        ...
        assert!(
            ...
            "SQLite stub must not use BIGSERIAL"
        );
    }
    ...
    fn render_reference_stub_tables_sql_skips_own_table() {
        ...
        let sql =
            render_reference_stub_tables_sql(DatabaseBackend::Postgres, &fields, "categories");
