...
pub fn plan_scaffold_with_options(
    ...
    if !options_with_key.api {
        ...
        // shared `crate::layout`, so the target app must expose one. Detect it
        // by looking for a `pub fn layout` in `src/main.rs` (as generated by
        ...
        if !main_src.contains("pub fn layout") {
            return Err(GenerateError::Config(
                "`autumn generate scaffold` requires a shared `pub fn layout` in src/main.rs \
                 ...
                 app created by `autumn new`, or add a shared \
