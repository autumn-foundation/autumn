...
/// Locate a shared `pub fn layout` in `src/main.rs` source (as generated by
/// `autumn new`) and return its top-level parameter count.
///
/// Returns `None` when no complete-token `pub fn layout` exists — so decoys
/// like `pub fn layout_sidebar` are ignored — or when the parameter list can't
/// be parsed (no matching close paren). `layout` is matched as a whole token
/// (the following character must be `(`, `<`, or whitespace). Top-level
/// parameters are counted by commas that are not nested inside `(...)`,
/// `[...]`, or `<...>`, so generic/tuple/array types in a parameter don't
/// inflate the count.
///
/// The scaffold's generated routes call the 4-arg
/// `crate::layout(title, current_path, flash, content)`, so callers require a
/// return of `Some(4)`; anything else (missing, or the wrong arity) is a
/// mismatch worth an early, actionable error.
fn shared_layout_param_count(main_src: &str) -> Option<usize> {
    const NEEDLE: &str = "pub fn layout";
    // Find `pub fn layout` where the char after `layout` is `(`, `<`, or
    // whitespace, so `layout_sidebar` and similar decoys don't count.
    let mut from = 0;
    let after_name = loop {
        let idx = main_src[from..].find(NEEDLE)? + from;
        let after = idx + NEEDLE.len();
        match main_src[after..].chars().next() {
            Some('(' | '<') => break after,
            Some(c) if c.is_whitespace() => break after,
            // A decoy like `layout_sidebar`: keep searching past this match.
            _ => from = after,
        }
    };
...
pub fn plan_scaffold_with_options(
    ...
    if !options_with_key.api {
        ...
        if !options_with_key.live && !options_with_key.live_validation {
            ...
            // layout must exist AND take exactly 4 parameters. `shared_layout_param_count`
            // matches `layout` as a complete token (so decoys like
            // `pub fn layout_sidebar` are ignored) and rejects an older/custom
            // 2-arg `pub fn layout(title, content)` that would otherwise pass the
            // preflight and then fail to compile.
            if shared_layout_param_count(&main_src) != Some(4) {
                return Err(GenerateError::Config(
