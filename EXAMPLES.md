...
| **Journey** | SPA + GraphQL on a real model: Autumn renders the page shell → a committed Vite/React 19/TypeScript bundle mounts into it → the bundle queries and mutates over GraphQL served by a generic `GraphqlPlugin` → every resolver goes through the generated `PgNoteRepository`, so normalisation, validation, and hooks behave identically for GraphQL, the generated REST handlers, and the startup seed |
| **Key capabilities** | `#[model]` with `#[normalize(trim)]` + `#[validate]`, `MutationHooks` (`before_create` validation, `before_delete` rule), `#[repository(hooks = …, api = …)]` with a derived finder and generated REST reads mounted beside GraphQL, `PgNoteRepository::with_pool_untracked` from `AppState` in resolvers and an `on_startup` seed, embedded migrations, `AutumnError` → GraphQL error with `extensions.status`, `Plugin` + `AppBuilder::nest` + `declare_plugin_routes` (audit-clean raw router), `PluginContract` + `plugin_conformance::run_conformance`, Maud shell + `asset_url` under the default `script-src 'self'` CSP, `autumn build --embed` single-binary deploy (`embed_static!` + `.embedded_static`), `GET /graphql/sdl` with a committed-SDL drift test, two-tier tests (`TestApp` without Docker; `TestDb` testcontainer with the real migration applied) |
| **Prerequisites** | Rust 1.88.0+, PostgreSQL (`docker compose up -d` in the example directory provides one); the React bundle is committed, so Node 20+ is needed only to change the frontend |
