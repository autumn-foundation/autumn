...
impl Route53Provider {
    ...
    async fn zone_id(&self, fqdn: &str) -> Result<String, String> {
        ...
        {
            return Ok(strip_zone_prefix(explicit).to_owned());
        }
            ...
            let body = self
                .send(
                    HttpRequest::new(
                        "GET",
                        format!(
                            "https://{API_HOST}{API_PREFIX}/hostedzonesbyname?dnsname={candidate}.&maxitems=1"
                        ),
                    ),
...
fn check_response(response: &HttpResponse, what: &str, url: &str) -> Result<String, String> {
...
mod tests {
    ...
    fn sigv4_matches_the_aws_get_vanilla_test_vector() {
        ...
        assert_eq!(
            ...
            // Re-derive what the signer hashed by rebuilding it the same way.
            sha256_hex(
                format!(
                    "GET\n/\n\nhost:example.amazonaws.com\nx-amz-date:20150830T123600Z\n\n\
                     host;x-amz-date\n{}",
