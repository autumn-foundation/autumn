...
mod tests {
    ...
    #[test]
    fn should_format_quota_exceeded() {
        ...
        assert_eq!(
            err.to_string(),
            "tenant 'test_tenant' memory quota exceeded: requested 100 bytes, 50 in use, quota 120 bytes"
        );
    }

    #[test]
    fn should_implement_default_and_is_empty_for_registry() {
        let registry = TenantCellRegistry::default();
        assert!(registry.is_empty());
        assert_eq!(registry.len(), 0);
        assert_eq!(registry.max_cells(), 0);
        assert_eq!(registry.idle_ttl(), None);
    }

    #[test]
    fn should_format_debug_structs() {
        let registry = TenantCellRegistry::default();
    ...
    #[test]
    fn should_clamp_release_at_zero() {
        let registry = TenantCellRegistry::default();
        let cell = registry.get_or_create("tenant1", 1000);
        cell.inner.release(500);
        assert_eq!(cell.tracked_bytes(), 0);
        assert_eq!(registry.total_tracked_bytes(), 0);
    }

    #[test]
    fn should_return_none_when_no_task_local_cell() {
        assert!(current_tenant_cell().is_none());
    }

    #[tokio::test]
    async fn should_resolve_current_tenant_cell_from_task_local() {
        let registry = TenantCellRegistry::default();
        let handle = TenantCellHandle::new(registry, "task_tenant".to_string(), 500);

        CURRENT_TENANT_CELL
            .scope(Some(handle.clone()), async move {
                let cell = current_tenant_cell().unwrap();
                assert_eq!(cell.tenant_id(), "task_tenant");
                assert_eq!(cell.quota_bytes(), 500);

                let cell2 = current_tenant_cell().unwrap();
    ...
    #[test]
    fn should_evict_and_return_cell() {
        let registry = TenantCellRegistry::default();
        let cell = registry.get_or_create("tenant_to_evict", 1000);
        assert!(!registry.is_empty());

        let evicted = registry.evict("tenant_to_evict").unwrap();
        assert!(registry.is_empty());
        assert!(Arc::ptr_eq(&cell, &evicted));

        assert!(registry.evict("non_existent").is_none());
    }

    #[test]
    fn should_return_existing_cell_on_get() {
        let registry = TenantCellRegistry::default();
        assert!(registry.get("t1").is_none());

        let created = registry.get_or_create("t1", 1000);
    ...
    #[test]
    fn should_handle_zero_capacity_quotas_on_reserve() {
        let registry = TenantCellRegistry::default();
        let cell = registry.get_or_create("zero_tenant", 0);

        let charge = cell.try_charge(10_000_000).unwrap();
        assert_eq!(charge.bytes(), 10_000_000);
        assert_eq!(cell.tracked_bytes(), 10_000_000);

        let _charge2 = cell.try_charge(20_000_000).unwrap();
        assert_eq!(cell.tracked_bytes(), 30_000_000);
    }

    #[test]
    fn should_evict_idle_older_than() {
        let registry = TenantCellRegistry::default();
        let cell = registry.get_or_create("idle_tenant", 1000);

        cell.touch(registry.now_millis(), registry.next_seq());

        std::thread::sleep(Duration::from_millis(10));

        let removed = registry.evict_idle_older_than(Duration::from_secs(10));
        assert_eq!(removed, 0);
        assert_eq!(registry.len(), 1);

        let removed = registry.evict_idle_older_than(Duration::from_millis(0));
        assert_eq!(removed, 1);
        assert!(registry.is_empty());
    }

    #[test]
    fn should_support_scratch_insert_get_remove() {
        let registry = TenantCellRegistry::default();
        let cell = registry.get_or_create("scratch_tenant", 10_000);

        assert!(cell.scratch_get("missing").is_none());
        assert!(cell.scratch_remove("missing").is_none());

        let val = vec![1, 2, 3];
        cell.scratch_insert("my_key", val.clone()).unwrap();

        assert_eq!(cell.scratch_get("my_key").unwrap(), val);

        let removed = cell.scratch_remove("my_key").unwrap();
        assert_eq!(removed, val);

        assert!(cell.scratch_get("my_key").is_none());
    }

    #[test]
    fn should_return_quota_exceeded_error_when_charging_above_quota() {
        let registry = TenantCellRegistry::default();
        let cell = registry.get_or_create("quota_tenant", 100);

        let _charge1 = cell.try_charge(50).unwrap();
        let _charge2 = cell.try_charge(50).unwrap();

        let err = cell.try_charge(1).unwrap_err();
        assert_eq!(err.tenant_id, "quota_tenant");
        assert_eq!(err.requested, 1);
        assert_eq!(err.in_use, 100);
        assert_eq!(err.quota, 100);
    }

    #[test]
    fn should_free_tracked_bytes_on_tenant_cell_inner_drop() {
        let global_tracked = Arc::new(AtomicUsize::new(0));
        {
            let cell = TenantCell::new("tenant1", 1000, global_tracked.clone());
            // forcibly reserve bytes inside cell
            cell.inner.reserve(500).unwrap();
            assert_eq!(global_tracked.load(Ordering::Relaxed), 500);

            // dropping cell should reclaim global tracked
        }

        assert_eq!(global_tracked.load(Ordering::Relaxed), 0);
    }
}
