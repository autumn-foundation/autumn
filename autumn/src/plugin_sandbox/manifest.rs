...
impl SandboxManifest {
    ...
    pub(crate) fn validate(&self) -> Result<(), ManifestError> {
        ...
        // grows.
        //
        // `with_capacity` from a caller-chosen length looks like the unbounded-allocation
        // defect this file refuses elsewhere, and today it is not one: `SandboxCapability`
        // has a single fieldless variant, so it is a ZST and a `Vec` of it never allocates
        // whatever the length says — measured, `size_of` is 0 and
        // `with_capacity(1_000_000)` takes 0 bytes. It becomes one the moment a second
        // variant lands and the type gains a byte, so whoever adds that variant should
        // size this from `SandboxCapability::ALL.len() + 1` instead.
