...
impl SandboxHost {
    ...
    pub fn run(&self, request: &SandboxRequest) -> SandboxOutcome {
        ...
        // Admission, before a single buffer is built. `serve` holds a permit of its
        // own across the whole request, so this never stops an ordinary HTTP request.
        // It is here for the embedder who calls this public method directly and would
        // otherwise start as many instances as it liked, against a footprint the
        // manifest validator accepted on the premise that `max_concurrency` bounds
        // them. The guard is held for the body of the run: dropping it on the way out
        // is what makes the permit mean "executing now".
        ...
        // Building the guest's stdin is host work proportional to the request: the body
        // is cloned into the frame and base64-expanded into the NDJSON line, all before
        // a single guest instruction runs. Unpriced, that is megabytes of host CPU per
        // request for a manifest declaring a large body ceiling and almost no fuel, and
        // a client can repeat it indefinitely. So it is charged at the same rate as
        // every other host-side copy, and charged before the copies happen: a budget
        // that cannot cover the encoding refuses without doing it. The guest then
        // starts on what is left, the same arrangement instantiation has.
