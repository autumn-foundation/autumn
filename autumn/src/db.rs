...
/// issues `BEGIN IMMEDIATE` before running `f`, so a concurrent writer queues on
/// the connection's `busy_timeout` instead of failing its deferred read→write
/// snapshot upgrade with `SQLITE_BUSY_SNAPSHOT` (which bypasses the busy
/// handler). It is the transaction primitive for generated write-RMW paths
/// (`with_lock`, `update`, `delete_by_id`, `find_or_create_by`); read-only
/// transactions, [`Db::tx`], and [`savepoint`] deliberately stay on the deferred
/// [`scoped_transaction`] so read-only user transactions keep their read
/// concurrency.
///
/// This is a runtime support function for code generated by Autumn proc macros.
/// It is semver-exempt; do not call it directly.
///
/// # SQLite nesting caveat
///
/// The manual `BEGIN IMMEDIATE` does not increment diesel's transaction-manager
/// depth counter, so `f` must not nest a [`savepoint`] or a
/// `TransactionManager`-driven `.transaction()` on the same connection — the
/// manager would think it is at depth 0 and emit a nested `BEGIN` (error) rather
/// than a `SAVEPOINT`. The adopted generated closures do no such nesting.
///
/// # Errors
///
...
/// transaction back before resuming the unwind, so the pooled connection is
/// never recycled with an open write transaction.
#[doc(hidden)]
pub async fn scoped_immediate_transaction<'a, T, E, C, F>(conn: &'a mut C, f: F) -> Result<T, E>
where
    C: diesel_async::AsyncConnection + Send,
    T: Send + 'a,
    E: From<diesel::result::Error> + Send + 'a,
        ...
        + 'a,
{
    crate::backend_select! {
        ...
        sqlite => {{
            // `batch_execute` comes from `SimpleAsyncConnection`, a supertrait of
            // the `C: AsyncConnection` bound, so it is in scope without an import;
            // the SQLite pool's `custom_setup` uses the same call for PRAGMAs.

            // Take the write lock up front. A concurrent writer queues here on
            // `busy_timeout` instead of failing a deferred snapshot upgrade.
            conn.batch_execute("BEGIN IMMEDIATE").await.map_err(E::from)?;

            // `catch_unwind` is mandatory: the manual BEGIN bypasses the
            // transaction-manager depth counter, so a panic that unwound without
            // a ROLLBACK would leave deadpool free to recycle this connection
            // with an open, uncommitted write transaction.
            let outcome = AssertUnwindSafe(f(&mut *conn)).catch_unwind().await;
            match outcome {
                Ok(Ok(value)) => {
                    // A COMMIT failure leaves the manual `BEGIN IMMEDIATE` txn
                    // open. Because we bypass diesel's transaction-manager depth
                    // counter, deadpool would see a "clean" TM and recycle this
                    // connection with the uncommitted write txn still open, so a
                    // later borrower would read uncommitted state or fail a nested
                    // BEGIN. Roll back before returning the commit error. There is
                    // no clean way to mark the pooled connection broken through the
                    // `&mut C` surface here, so if even the ROLLBACK fails we log
                    // loudly and still surface the commit error (documented
                    // limitation: the connection may be recycled dirty in that rare
                    // double-failure case).
                    if let Err(commit_err) = conn.batch_execute("COMMIT").await {
                        if let Err(rollback_err) = conn.batch_execute("ROLLBACK").await {
                            tracing::error!(
                                "failed to roll back immediate transaction after COMMIT failure: {rollback_err}"
                            );
                        }
